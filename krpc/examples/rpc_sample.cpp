...
int my_rpc_func_stub(krpc_object_t* o) {
    ...
    std::string p1;
    p1 = krpc_string_get(krpc_vector_get(o, 1));
    return my_rpc_func(p0, p1);
}
...
int rpc_sample_t::my_rpc_func(stream_t* stream, my_object_t& my_obj, const std::string& test_str) {
    return krpc_call(_rpc, stream, 1, my_rpc_func_proxy(my_obj, test_str));
}
